...
class Usuario(Base):
    ...
    sede_id = Column(Integer, ForeignKey('sedes.id'))
    empleado_id = Column(Integer, ForeignKey('empleados.id'))
    sede_actual_id = Column(Integer, ForeignKey('sedes.id'))
    sede_actual = relationship(
        "Sede", 
        back_populates="usuarios_actuales",
        foreign_keys=[sede_actual_id]
    )
    ...
    permisos_consulta = Column(String(255))
    permisos_inventario = Column(String(255))
