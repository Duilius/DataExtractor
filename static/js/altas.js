...
async function loadPageData(page) {
    try {
        const response = await fetch(`/dashboard/inventariador/bienes-altas-2024/data?page=${page}&limit=${currentState.pageSize}`);
...
async function searchBienes(event) {
    ...
    try {
        const response = await fetch('/dashboard/inventariador/search-bienes', {
            method: 'POST',
            body: formData
        });
...
function showModal(codigoPatrimonial) {
    const row = document.querySelector(`tr[data-codigo="${codigoPatrimonial}"]`);
    if (!row) return;

    const cells = Array.from(row.cells);
    const bien = {
        codigo_patrimonial: cells[0].textContent.trim(),
        codigo_sbn: cells[1].textContent.trim(),
        denominacion: cells[2].textContent.trim(),
        color: cells[3].textContent.trim(),
        marca: cells[4].textContent.trim(),
        modelo: cells[5].textContent.trim(),
        numero_serie: cells[6].textContent.trim()
    };
    ...
    modalContent.innerHTML = `
        <div class="grid grid-cols-2 gap-4">
            ...
            <div>
                ...
                <p class="mt-1">${bien.codigo_patrimonial}</p>
            </div>
            <div>
            ...
            <div>
                <strong class="text-gray-600">Color</strong>
                <p class="mt-1">${bien.color}</p>
            </div>
            <div>
            ...
            <div>
                ...
                <p class="mt-1">${bien.numero_serie}</p>
            </div>
        </div>
    `;
...
function loadNextPage() {
    if (currentState.isSearching) {
        ...
        fetch('/dashboard/inventariador/search-bienes', {
            method: 'POST',
            body: formData
        })
